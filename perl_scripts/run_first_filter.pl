...
foreach my $maf (@mafs){
  foreach my $miss (@misses){
    ...
    push @slurmdirectives, "module load arcc/1.0 gcc/12.2.0 vcftools/0.1.16";
    push @slurmdirectives, "cd $vcf_path";
    push @slurmdirectives, "vcftools --vcf $file --out variants_maf"."$maf"."_miss"."$miss"." --remove-filtered-all --maf 0.0"."$maf"." --max-missing 0."."$miss"." --recode --thin 100";
