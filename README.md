...
Both sauger_cross_check_trios.R and sauger_cross_check_pars.R ran successfully, so I'm now curious about adding in missing data. Should we write in a section of code that says, "For each site, take a random draw from a distribution of missing data that corresponds to the results of the % MD per site analysis. Then delete that proportion of cells from each site." THEN run GetMaybeRel() on it and then the check scripts. I mean, that makes sense, but maybe do it at the mean and quantiles of the % MD per site distribution instead of for a bunch of random values? Let's see what Josh says. Going to message him now.
